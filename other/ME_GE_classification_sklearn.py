...
def main(jobid, lr_opt, batch_size, l1_value, modeltype, pheno_name, fold):
    ...
    model = skl.LogisticRegression(penalty='l1', solver='liblinear', n_jobs = 10, C=l1_value) 
    #sklearn.linear_model.Lasso(, alpha=l1_value)
    ...
    print("auc val ge ", auc_val)
    print(cm_val)

    ptest = model.predict(xtest_GE)
    np.save(rfrun_path + "/ptest_ge.npy", ptest)
    auc_test, cm_test = evaluate_performance(ytest, ptest)
    print("auc test ge ", auc_test)
    print(cm_test)
    
    np.save(rfrun_path + "/auc_val_ge.npy", auc_val)
    np.save(rfrun_path + "/auc_test_ge.npy", auc_test)
    
    auc_val_GE = auc_val
    auc_test_GE = auc_test
    ...
    auc_val, cm_val = evaluate_performance(yval, pval)
    print("auc val me ", auc_val)
    print(cm_val)

    ptest = model.predict(xtest_ME)
    np.save(rfrun_path + "/ptest_me.npy", ptest)
    auc_test, cm_test = evaluate_performance(ytest, ptest)
    print("auc test me ", auc_test)
    print(cm_test)
    
    np.save(rfrun_path + "/auc_val_me.npy", auc_val)
    np.save(rfrun_path + "/auc_test_me.npy", auc_test)

    auc_val_ME = auc_val
    auc_test_ME = auc_test
    print(rfrun_path)
